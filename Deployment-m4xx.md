...
The API Docker healthcheck uses `curl`. If your runtime image lacks `curl`, healthcheck can fail even when app runs.

Fix options:

- install `curl` in API Docker image
- or adjust healthcheck command to use a tool available in image
